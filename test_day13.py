from day13 import Vector, Machine, fewest_tokens_for_all_possible_prizes


def test_valid_cost():
    prize = Vector(8400, 5400)
    buttonA = Vector(94, 34)
    buttonB = Vector(22, 67)

    cost = prize.cost(buttonA, buttonB)
    assert cost == 280

    assert Vector(7870, 6450).cost(Vector(17, 86), Vector(84, 37)) == 200


def test_fractional_presses_no_cost():
    assert Vector(12748, 12176).cost(Vector(26, 66), Vector(67, 21)) is None

    assert Vector(18641, 10279).cost(Vector(69, 23), Vector(27, 71)) is None


def test_same_direction_missing_prize_no_cost():
    prize = Vector(10, 1)
    buttonA = Vector(1, 2)
    buttonB = Vector(2, 4)
...
def test_same_direction_hitting_prize():
    prize = Vector(12, 12)
    buttonA = Vector(1, 1)
    buttonB = Vector(5, 5)
...
def test_same_direction_missing_prize():
    prize = Vector(12, 12)
    buttonA = Vector(5, 5)
    buttonB = Vector(5, 5)
...
def test_same_direction_out_of_line_with_prize():
    prize = Vector(12, 10)
    buttonA = Vector(1, 1)
    buttonB = Vector(5, 5)
...
def test_machine_parse_many():
    ms = Machine.parse_many(test_input)
    assert [m.cost() for m in ms] == [280, None, 200, None]


def test_fewest_tokens():
    ms = Machine.parse_many(test_input)
    assert fewest_tokens_for_all_possible_prizes(ms) == 480


def test_real_fewest_tokens():
    ms = Machine.parse_many(real_input)
    assert fewest_tokens_for_all_possible_prizes(ms) == 39748


def test_corrected_tokens():
    ms = Machine.parse_many(test_input)
    ms = [m.with_corrected_prize() for m in ms]
    assert [m.cost() for m in ms] == [None, 459236326669, None, 416082282239]


def test_real_corrected_fewest_tokens():
    ms = Machine.parse_many(real_input)
    ms = [m.with_corrected_prize() for m in ms]
    assert fewest_tokens_for_all_possible_prizes(ms) == 74478585072604
